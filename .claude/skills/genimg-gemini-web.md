...
```bash
npx -y bun skills/genimg-gemini-web/scripts/main.ts "Hello, Gemini"
npx -y bun skills/genimg-gemini-web/scripts/main.ts --prompt "Explain quantum computing"
npx -y bun skills/genimg-gemini-web/scripts/main.ts --prompt "A cute cat" --image cat.png
...
npx -y bun skills/genimg-gemini-web/scripts/main.ts "Remember this: 42" --sessionId my-unique-id-123
npx -y bun skills/genimg-gemini-web/scripts/main.ts "What number?" --sessionId my-unique-id-123
```

## Executor options (programmatic)

This skill is typically consumed via `createGeminiWebExecutor(geminiOptions)` (see `skills/genimg-gemini-web/scripts/executor.ts`).
...
npx -y bun skills/genimg-gemini-web/scripts/main.ts "Your prompt here"

# Explicit prompt flag
npx -y bun skills/genimg-gemini-web/scripts/main.ts --prompt "Your prompt here"
npx -y bun skills/genimg-gemini-web/scripts/main.ts -p "Your prompt here"

# With model selection
npx -y bun skills/genimg-gemini-web/scripts/main.ts -p "Hello" -m gemini-2.5-pro

# Pipe from stdin
echo "Summarize this" | npx -y bun skills/genimg-gemini-web/scripts/main.ts
```
...
npx -y bun skills/genimg-gemini-web/scripts/main.ts --prompt "A sunset over mountains" --image

# Generate image with custom path
npx -y bun skills/genimg-gemini-web/scripts/main.ts --prompt "A cute robot" --image robot.png

# Shorthand
npx -y bun skills/genimg-gemini-web/scripts/main.ts "A dragon" --image=dragon.png
```
...
# JSON output
npx -y bun skills/genimg-gemini-web/scripts/main.ts "Hello" --json
```
...
CLI note: `skills/genimg-gemini-web/scripts/main.ts` supports text generation, image generation, and multi-turn conversations via `--sessionId`. Reference images and video generation are exposed via the executor API.
...
npx -y bun skills/genimg-gemini-web/scripts/main.ts --login
```
...
```bash
npx -y bun skills/genimg-gemini-web/scripts/main.ts "What is the capital of France?"
```

### Generate image
```bash
npx -y bun skills/genimg-gemini-web/scripts/main.ts "A photorealistic image of a golden retriever puppy" --image puppy.png
```

### Get JSON output for parsing
```bash
npx -y bun skills/genimg-gemini-web/scripts/main.ts "Hello" --json | jq '.text'
```

### Generate image from prompt files
```bash
# Concatenate system.md + content.md as prompt
npx -y bun skills/genimg-gemini-web/scripts/main.ts --promptfiles system.md content.md --image output.png
```

### Multi-turn conversation
```bash
# Start a session with unique ID (agent generates this)
npx -y bun skills/genimg-gemini-web/scripts/main.ts "You are a helpful math tutor." --sessionId task-abc123

# Continue the conversation (remembers context)
npx -y bun skills/genimg-gemini-web/scripts/main.ts "What is 2+2?" --sessionId task-abc123
npx -y bun skills/genimg-gemini-web/scripts/main.ts "Now multiply that by 10" --sessionId task-abc123

# List recent sessions (max 100, sorted by update time)
npx -y bun skills/genimg-gemini-web/scripts/main.ts --list-sessions
```
